...
| Files on disk being wrong | `Common/Roadmap.md` → **Library** table |
...
**Three kinds of work, tracked separately.** A **bug** is the app not doing what
it already promises. A **feature** is something the app does not do yet. A
**library** item is the collection on disk being wrong, where the fix is to the
files and no code changes. When something is arguably both a bug and a feature,
file it as a bug — "is this broken?" is the question that decides how soon it
gets looked at.

The library table exists because the alternative is worse: filed as a bug, a
mis-sorted show implies code is at fault and sends the next session reading
`identify.py` for a defect that is not there; filed as a feature, it never gets
done because it builds nothing. It is also the one kind that closes **without** a
CHANGELOG line — nothing about the app changed, so there is nothing to tell a
user, and the commit that corrected the files is the record.

### Item IDs

`F-DDMM.NN` for a feature, `B-DDMM.NN` for a bug, `L-DDMM.NN` for a library
item: the kind, the day it was raised, and a counter within that day.
`F-0108.01` is the first feature raised on 1 August. The counter restarts daily,
so it only has to be unique within its own date — no need to scan the whole file
...
looks like it. One that turns out to be a different kind keeps its number and
swaps its letter — `F-0108.06` became `L-0108.06` when Batman's Season 04
turned out to be a shelving problem rather than something to build.
...
Bugs and features both land in the same CHANGELOG, which is written for a user
and so groups by what changed for them, not by which table the work came from.
Library items produce no CHANGELOG line at all. A fixed bug reads "Fixed …"; a
shipped feature just says what it now does.

**A shipped item is deleted from the Roadmap, not marked done** — the CHANGELOG
is the record of what shipped. Never delete a bug or feature row until the
CHANGELOG covers it, or the item vanishes from the record entirely. A library
row is deleted once the files are right, and the commit that corrected them is
its record.
