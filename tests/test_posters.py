...
POSTERS = [
    {'language': 'en', 'width': 2000, 'height': 3000, 'vote_average': 8.0, 'vote_count': 6,
     'thumb_url': 'https://image.tmdb.org/t/p/w342/a.jpg',
     'poster_url': 'https://image.tmdb.org/t/p/w500/a.jpg'},
    {'language': None, 'width': 1000, 'height': 1500, 'vote_average': 5.0, 'vote_count': 1,
     'thumb_url': 'https://image.tmdb.org/t/p/w342/b.jpg',
     'poster_url': 'https://image.tmdb.org/t/p/w500/b.jpg'},
    {'language': 'fr', 'width': 1000, 'height': 1500, 'vote_average': 4.0, 'vote_count': 2,
...
app.store.add_media_item(imdb_id='tt111', tmdb_id=42, title='A Film', year=2001,
                         media_type='movie', collection_id=None, collection_name=None,
                         current_quality='1080p', path=None, poster_url='/static/posters/old.jpg')
mid = app.store.get_media_item_by_imdb_id('tt111')['id']

app.store.add_media_item(imdb_id='tt222', tmdb_id=None, title='No Tmdb', year=2002,
                         media_type='movie', collection_id=None, collection_name=None,
                         current_quality=None, path=None)
...
for bad, label in [
    ...
    check(f'rejects {label}', r.status_code == 400 and len(downloaded) == before,
          f'{r.status_code} downloads={len(downloaded) - before}')

print('\n=== 3. Saving a chosen poster ===')
before = app.store.get_media_item(mid)['poster_url']
r = c.post(f'/api/library/{mid}/poster',
           json={'poster_url': 'https://image.tmdb.org/t/p/w500/a.jpg'})
...
r = c.post(f'/api/library/{mid}/poster',
           json={'poster_url': 'https://image.tmdb.org/t/p/w500/c.jpg'})
