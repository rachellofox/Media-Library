...
check(
    'a part word is required, so "I Am Legend" keeps its I',
...
episodes = [
    ep(1, 3, 'Enter Magneto'),
    ep(1, 1, 'Night of the Sentinels (1)'),
...
check(
    'a bare title matching two parts is refused',
    got is None and reason == 'ambiguous',
    (got, reason),
)
...
saga = [
    ep(3, 3, 'The Phoenix Saga: Sacrifice (1)'),
...
check(
    'both point at the same episode',
...
check(
    'one bare title against three unclaimed parts stays unresolved',
    'Story.mkv' not in resolved,
    resolved,
)
resolved = match_episode_files(['Story.mkv', 'Story Pt. 2.mkv', 'Story Pt. 3.mkv'], three_parts)
check(
    'but resolves once the others are claimed',
    resolved.get('Story.mkv', {}).get('episode_number') == 1,
    resolved.get('Story.mkv'),
)
...
for path, expected in [
    ...
    check(
        f'{path[:38]!r} extras={expected}', is_extras_path(path) is expected, is_extras_path(path)
    )
