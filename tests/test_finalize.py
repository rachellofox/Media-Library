...
def add_item(imdb, title, year, path, quality):
    app.store.add_media_item(
        imdb_id=imdb, tmdb_id=None, title=title, year=year, media_type='movie',
        collection_id=None, collection_name=None, current_quality=quality,
        path=path,
    )
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    app.store.set_download_state(mid, 'downloading', 'qb_webui', 'x',
                                 torrent_hash='A' * 40, mode='upgrade',
                                 previous_path=old_folder)

    torrent = {'state': 'stalledUP', 'progress': 1.0, 'amount_left': 0,
               'content_path': dl_folder}
    ...
    check('canonical name uses TMDB year 2018 not release year 2015',
    ...
    check('DB path points at canonical folder',
    ...
    check('library file is a hardlink to the seeded file',
          os.path.exists(want_file) and os.stat(want_file).st_nlink >= 2)
    check('quality updated to 2160p',
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    app.store.set_download_state(mid, 'downloading', 'qb_webui', 'x',
                                 torrent_hash='B' * 40, mode='upgrade',
                                 previous_path=old_folder)
    ...
    check('flagged needs_review', row_for(mid)['download_status'] == 'needs_review',
...
with tempfile.TemporaryDirectory() as tmp:
    lib, staging, recycled = setup(tmp)
    partial = {'state': 'downloading', 'progress': 0.4, 'amount_left': 900,
               'content_path': staging}
    check('downloading state rejected', not app._torrent_is_complete(partial))
    check('seeding but bytes left rejected', not app._torrent_is_complete(
        {'state': 'stalledUP', 'progress': 1.0, 'amount_left': 4096, 'content_path': staging}))
    check('seeding but progress<1 rejected', not app._torrent_is_complete(
        {'state': 'seeding', 'progress': 0.99, 'amount_left': 0, 'content_path': staging}))
    check('checkingUP with full bytes accepted', app._torrent_is_complete(
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    app.store.set_download_state(mid, 'downloading', 'qb_webui', 'x',
                                 torrent_hash='D' * 40, mode='upgrade',
                                 previous_path=old_folder)
    ...
    check('canonical file is now the 4K one',
    ...
    check('no .incoming leftover',
          not any(f.endswith('.incoming') for f in os.listdir(old_folder)),
          str(os.listdir(old_folder)))
    check('exactly one video in folder', len(os.listdir(old_folder)) == 1,
          str(os.listdir(old_folder)))
    check('quality now 2160p',
          app.store.get_media_item(mid)['current_quality'] == '2160p',
          str(app.store.get_media_item(mid)['current_quality']))
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    app.store.set_download_state(mid, 'downloading', 'qb_webui', 'x',
                                 torrent_hash='E' * 40, mode='upgrade',
                                 previous_path=old_folder)
    ...
    check('original file preserved when recycling impossible',
