...
BROADCAST = [ep(1, 1, 'The Train Job', 43), ep(1, 2, 'Bushwhacked', 44),
             ep(1, 3, 'Our Mrs. Reynolds', 44), ep(1, 11, 'Serenity', 87)]
# The DVD ordering: the pilot first, everything else shifted along.
DVD = [ep(1, 1, 'Serenity', 87), ep(1, 2, 'The Train Job', 43),
       ep(1, 3, 'Bushwhacked', 44), ep(1, 6, 'Our Mrs. Reynolds', 44)]
...
check('E01 is now Serenity, not The Train Job',
      next(e['title'] for e in chosen if e['episode_number'] == 1) == 'Serenity')
check('DVD wins the tie over Absolute, so the choice is repeatable',
      notes and notes[0][1] == 'DVD Order', notes)
...
flat_probes = {'a.mp4': (22.0, 'On Leather Wings'),
               'b.mp4': (22.0, 'Christmas with the Joker')}
...
namespace['app'] = type('A', (), {'tmdb': FakeTmdb([
    {'id': 'y', 'name': 'DVD Order', 'kind': 'DVD', 'episodes': flat_dvd}])})()
notes, left = [], []
...
check('and the reason given is the embedded titles',
      notes and 'carry the episode title' in notes[0][2], notes)
...
for junk in ['WENTWORTH Series 2 Disc 2', 'S01D01title_t02',
             'movieddl.me_Dollhouse.S01E01.1080p.Bluray', 'Mr Robot S01E01 hellofriend.mov']:
    titles, _c, _a, disagree = score({'x.mkv': [(1, 1)]}, [ep(1, 1, 'Ghost', 50)],
                                     {'x.mkv': (50.0, junk)})
    check(f'{junk[:34]!r} neither matches nor contradicts',
          (titles, disagree) == (0, 0), (titles, disagree))
...
notes, left = [], []
chosen, untitled = resolve('Parks', 8592, {'s06e20.mkv': [(6, 20)]},
                           [ep(6, 20, 'Moving Up', 44)], notes, left)
check('the season is marked untitled', untitled == {6}, untitled)
check('reported as left alone, not as a detected ordering',
      left and not notes, (notes, left))
