...
def make_film(imdb, title, quality):
    ...
    with open(os.path.join(folder, f'{title} (2001).mkv'), 'wb') as fh:
        fh.write(b'\x00' * 4096)
    app.store.add_media_item(imdb_id=imdb, tmdb_id=None, title=title, year=2001,
                             media_type='movie', collection_id=None, collection_name=None,
                             current_quality=quality, path=folder, poster_url='/x.jpg',
                             synopsis='s')
...
for media_id in (idle_id, busy_id):
    app.store.add_quality_check(media_item_id=media_id, best_found_quality='2160p',
                                best_found_name='rel', best_found_desc_link='x',
                                found=True, raw_result_count=1)
app.store.set_download_state(media_item_id=busy_id, status='downloading',
                             source='qb_webui', message='x', torrent_hash='A' * 40)
...
for label, media_id in (('idle', idle_id), ('downloading', busy_id)):
    row = rows[media_id]
    print(f"  {label:12} upgrade_available={app._upgrade_available(row)} "
          f"status={row['download_status']!r}")
check('both items genuinely have an upgrade available',
...
check('downloading film does NOT show the upgrade badge',
...
check('hero excludes downloading items',
      'libraryItemDownloading(item)' in condition, condition.replace('\n', ' ')[:120])
