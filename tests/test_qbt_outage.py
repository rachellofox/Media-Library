...
def stale(mid):
    ...
    with app.store.conn() as c:
        c.execute(
            "UPDATE download_states SET updated_at = datetime('now','-2 days') "
            'WHERE media_item_id = ?',
            (mid,),
        )
...
check(
    ...
    (row_for(mid)['download_status'] or '') == 'downloading',
    row_for(mid)['download_status'],
)
check(
    'item still visible in the library view',
    bool((row_for(mid)['path'] or '').strip())
    or row_for(mid)['download_status'] in {'starting', 'handed_off', 'downloading'},
)
...
app._auto_finalize_qb_completed_downloads()
check(
    'stale state cleared when qB really has nothing',
    (row_for(mid)['download_status'] or '') == '',
    row_for(mid)['download_status'],
)
...
running = [
    {
        'hash': 'B' * 40,
        'name': 'The Terminator 1984 2160p Bluray x265 KiNGDOM',
        'state': 'downloading',
        'progress': 0.24,
        'amount_left': 999,
...
check(
    'torrent hash recorded',
    (r['download_torrent_hash'] or '') == 'B' * 40,
...
app._auto_finalize_qb_completed_downloads()
check(
    '1991 item not adopted by the 1984 torrent',
    (row_for(mid2)['download_status'] or '') == '',
    row_for(mid2)['download_status'],
)
...
app._auto_finalize_qb_completed_downloads()
check(
    'item with a local file not re-adopted',
    (row_for(mid3)['download_status'] or '') == '',
    row_for(mid3)['download_status'],
)
