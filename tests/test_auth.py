...
check(
    'api returns 401 JSON, not a redirect',
    c.get('/api/library/retry-download/1').status_code == 401,
)
check(
    'fetch-style request returns 401',
...
c = client()
codes = [
    c.post('/login', data={'username': 'rachel', 'password': 'nope'}).status_code
...
r = c.post(
    '/login',
    data={'username': 'rachel', 'password': 'correct-horse', 'next': 'https://evil.example/steal'},
)
check(
    'sign-in ignores an off-site next',
...
c = client()
r = c.post(
    '/settings', data={'public_access_submitted': '1', 'public_access': '1', 'server_port': '5100'}
)
check(
    'rejected with auth_required_for_public',
    'auth_required_for_public' in r.headers.get('Location', ''),
    r.headers.get('Location'),
)
...
r = c.post(
    ...
    data={
        'public_access_submitted': '1',
        'auth_username': 'rachel',
        'auth_password': 'longenough1',
        'auth_password_confirm': 'different1',
        'server_port': '5100',
    },
)
...
r = c.post(
    ...
    data={
        ...
        'auth_username': 'rachel',
        'auth_password': 'longenough1',
        'auth_password_confirm': 'longenough1',
        'server_port': '5100',
    },
)
check(
    'valid credentials + public access accepted',
    'public_access_saved' in r.headers.get('Location', ''),
    r.headers.get('Location'),
)
...
check(
    'settings now require signing in',
    r.status_code == 302 and '/login' in r.headers.get('Location', ''),
)
r = c.post('/settings', data={'public_access_submitted': '1', 'auth_username': 'hacker'})
check(
    'settings POST is gated too', r.status_code == 302 and '/login' in r.headers.get('Location', '')
)
...
check(
    'old session no longer valid after username change',
